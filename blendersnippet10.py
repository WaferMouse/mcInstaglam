...
logdir = os.path.expanduser('~/mcobjcmd/') # location of log files generated by Skript
worlddir = os.path.expanduser('~/worlds/') # where are worlds stored?
outputdir = os.path.expanduser('~/renders/') # where should our renders go?
workingdir = os.path.expanduser('~/igtemp/') # where should we temporarily store the .obj files?
jmc2obj = os.path.expanduser('~/jmc2obj/jMc2Obj-dev_r276M.jar') # location of jmc2obj .jar
...
def addObj(obj):
    x1,z1,x2,z2 = obj
#    call(["java", "-jar", jmc2obj, "-s", "--objfile=x"+str(x1)+"-z"+str(z1)+".obj", "--mtlfile=x"+str(x1)+"-z"+str(z1)+".mtl", "--output="+workingdir, "--height=50,256", "--area="+str(x1)+","+str(z1)+","+str(x2+32)+","+str(z2+64),worlddir+worldname]) #this does the actual export
    objlist.append(obj)
    return()
