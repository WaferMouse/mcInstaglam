...
def find_chunks4(pos, yaw, viewdistance, fov):
    ...
    x2,z2 = point_finder(xq,zq, ((yaw+270) % 360), (viewdistance)*math.tan((fov*(math.pi/180))*0.725)) # something's going on with the FOV that I can't fucking figure out
...
def placeCamera(player, viewdistance, fov):
    location, rotation = player # narf!
    bpy.ops.object.camera_add(location=location, rotation=rotation)
    cam = bpy.data.cameras[len(bpy.data.cameras)-1]
#    cam.lens = (cam.sensor_width/2)/math.tan(fov)
#    cam.lens = (cam.sensor_width * math.tan(((fov)*math.pi/180)/2))/2
...
def addCamera(player, viewdistance, fov):
    camera = player, viewdistance, fov
...
for row in ofile:
    ...
    addCamera(mc_player((x,y,z),dirty_trig(targetx,targety,targetz)),viewdistance,fov)
    placeCamera(mc_player((x,y,z),dirty_trig(targetx,targety,targetz)),viewdistance,fov)
csvfile.close()
